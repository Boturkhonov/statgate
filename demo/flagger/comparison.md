...
## Сценарий 0 — Baseline (happy path)

**Файлы:** [`demo/scenarios/00-baseline/`](../scenarios/00-baseline/)

**Гипотеза.** Canary эквивалентен stable по error rate (0% у обоих). Оба инструмента должны без колебаний promote.

**Setup:** stable `ERROR_RATE=0`, canary `ERROR_RATE=0`. StatGate `delta=0.05` (стандарт), Flagger `thresholdRange.max=5` (стандарт).

**Ожидание.**
- StatGate накапливает отрицательные success-инкременты Λ, достигает B на каждом шаге, продвигается. **Promoted**.
- Flagger видит error rate 0% во всех окнах. **Succeeded**.

**Аргумент защиты.** Базовая проверка корректности: оба инструмента **не блокируют здоровые релизы**. Без этого кейса не доказана α-гарантия StatGate (вероятность ложного отката ≤ 5%) — даже простая правильность подхода под вопросом.
...
**Фактические результаты (прогон 2026-05-24):**

| Метрика | StatGate | Flagger |
|---|---|---|
| Финальное состояние | _(заполнить после прогона)_ | **Succeeded** (canary promoted) |
| Время до решения | _(заполнить)_ | ~4 мин 30 с от apply Canary CR |
| Шаг (вес canary), на котором принято решение | _(заполнить)_ | прошёл 20% → 40% → 60% → Promotion |
| Наблюдаемый error rate | _(заполнить)_ | ~1.5–4% (rate-smoothed); порог 5% не превышен |
| Failed checks | _(не применимо)_ | 0 |

> **Вывод по Flagger.** На реально деградированной (4.5%) canary-версии порог `thresholdRange.max=5` не сработал ни в одном из 60-секундных окон. Версия promoted в прод. Это и есть false-negative по определению.
...
|---|---|---|---|---|
| 00 Baseline | Корректность в норме (α-гарантия) | Succeeded | Promoted | ★ обязательная проверка |
